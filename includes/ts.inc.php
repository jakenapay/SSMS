...
if (isset($_POST['save-changes'])) {
    ...
    $updateQuery = "UPDATE epiz_33456032_ssms.technology_supplies SET `ts_name`='$name', `ts_model`='$model', `ts_brand`='$brand', `ts_category`='$cat', `ts_quantity`='$qty', `ts_location`='$loc', `ts_desc`='$des', `date_last_modified`='$now',`modified_by`=$uid WHERE ts_id=$tsid;";
...
if (isset($_POST['get-btn-tech'])) {
    ...
    if ($conn->query($sql) === TRUE) {
        $sql2 = "INSERT INTO epiz_33456032_ssms.history(`ts_id`, `history_quantity`, `user_id`, `status`, `modified_by`, `history_date`) VALUES ('$tsid', '$qty', $id, 'pending', NULL, now())";
