...
if (isset($_GET['eid']) and ($_GET['eid']) != '') {
    ...
    $id = $_GET['eid'];
    $result = $conn->query("SELECT *, technology_supplies.date_last_modified as dlm, CONCAT(users.user_firstname, ' ', users.user_lastname) as fullname FROM technology_supplies INNER JOIN users ON technology_supplies.modified_by=users.user_id WHERE ts_id = $id LIMIT 1");
